<# 
Apigee hybrid bootstrap (Control Plane) - PRODUCCIÓN - Windows PowerShell
Crea/actualiza recursos del control plane en GCP para el entorno prod:
- Habilita APIs (apigee + apigeeconnect)
- Crea ORG (runtimeType=HYBRID) si no existe y espera a que termine la operación
- Crea Environment (prod) si no existe
- Crea EnvGroup (eg-prod) si no existe
...
param(
  ...
  [string]  $EnvGroupName = "eg-prod",
  [Parameter(Mandatory=$true)] [string[]]$Hostnames,            # ej: @("api-prod.empresa.com")
...
if (-not $envExists) {
  $envBody = @{ displayName = $EnvironmentName; description = "Environment $EnvironmentName (bootstrap prod)" }
