<# 
Apigee hybrid bootstrap (Control Plane) - TEST - Windows PowerShell
Crea/actualiza recursos del control plane en GCP para el entorno test:
- Habilita APIs (apigee + apigeeconnect)
- Crea ORG (runtimeType=HYBRID) si no existe y espera a que termine la operación
- Crea Environment (test) si no existe
- Crea EnvGroup (eg-test) si no existe
...
param(
  ...
  [string]  $EnvGroupName = "eg-test",
  [Parameter(Mandatory=$true)] [string[]]$Hostnames,            # ej: @("api-test.empresa.com")
...
if (-not $envExists) {
  $envBody = @{ displayName = $EnvironmentName; description = "Environment $EnvironmentName (bootstrap test)" }
